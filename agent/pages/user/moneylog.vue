...
<script>
	import { getUserMoneyLog } from '@/api/agent.js'
	...
	export default {
		data() {
			...
			userInfo: null,
			// 分类列表
			categoryList: MONEY_LOG_TYPES.CATEGORY_LIST1,
			currentCategory: 0,
	...
	onLoad(options) {
		this.memberId = options.member_id || null;
		if(options.username) {
			uni.setNavigationBarTitle({
				title: options.username+'的资金记录' 
			});
		}
		this.initData()
	},
	...
	setDefaultDate() {
		...
		async loadMoneyLog() {
			try {
				...
				const res = await getUserMoneyLog(params)
			if (res.code === 1) {
