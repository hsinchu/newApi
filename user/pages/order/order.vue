...
import { getBetOrders } from '@/api/bet/bet.js';
	...
	export default {
		...
		methods: {
			...
			order.betTypeName = order.bet_type_name;
				...
				order.createTime = this.formatDateTime(order.create_time);
			...
			async fetchOrderData(page = 1, isLoadMore = false) {
				...
				try {
					...
					const response = await getBetOrders(params);
			...
			onSelectMenu(e) {
				...
				// H5环境下的调试信息
				// #ifdef H5
				console.log('H5环境 - 下拉菜单事件:', e);
				console.log('H5环境 - 当前视口信息:', {
			...
			onPopupChange(e) {
				...
				// #ifdef H5
				if (e.show) {
					console.log('H5环境 - 下拉菜单显示');
					// 确保弹窗在正确位置显示
					this.$nextTick(() => {
						const popup = document.querySelector('.uv-drop-down-popup');
						if (popup) {
							popup.style.display = 'block';
							popup.style.visibility = 'visible';
							console.log('H5环境 - 强制显示下拉菜单');
						}
					});
				} else {
					console.log('H5环境 - 下拉菜单隐藏');
				}
				// #endif
			},
...
<style scoped lang="scss">
	...
	.bet-content-text{font-size:25rpx;color:orangered;}
