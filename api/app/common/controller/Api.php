...
class Api extends BaseController
{
     ...
     */
    protected function success(string $msg = '', mixed $data = null, int $code = 1, string $type = null, array $header = [], array $options = []): never
    {
        // 记录成功日志（可选）
        if (config('app.debug')) {
            trace('API Success: ' . $msg, 'info');
        }
     ...
     * @throws HttpResponseException
     */
    protected function error(string $msg = '', mixed $data = null, int $code = 0, string $type = null, array $header = [], array $options = []): never
    {
        // 记录错误日志
        if (config('app.debug')) {
            trace('API Error: ' . $msg, 'error');
        }
     ...
     * @throws HttpResponseException
     */
    public function result(string $msg, mixed $data = null, int $code = 0, string $type = null, array $header = [], array $options = []): never
    {
        // 统一响应格式
        $result = [
            'code' => $code,
        ...
        if (config('app.debug')) {
            $result['debug'] = [
                ...
                'execution_time' => microtime(true) - $this->request->server('REQUEST_TIME_FLOAT'),
    ...
    public function jsonReturn($msg, $data = [], $code = 1){
        $result = [
            'code' => $code,
            'msg'  => $msg,
            'time' => $this->request->server('REQUEST_TIME'),
